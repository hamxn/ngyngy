...
c(v).css("zIndex",x.slideCount+(s===true?0:1));};c.fn.cycle.custom=function(G,A,s,w,y,u){var F=c(G),B=c(A);var v=s.speedIn,E=s.speedOut,x=s.easeIn,D=s.easeOut,t=s.animInDelay,z=s.animOutDelay;
B.css(s.cssBefore);if(u){if(typeof u=="number"){v=E=u;}else{v=E=1;}x=D=null;}var C=function(){B.delay(t).animate(s.animIn,v,x,function(){w();});};F.delay(z).animate(s.animOut,E,D,function(){F.css(s.cssAfter);
if(!s.sync){C();}});if(s.sync){C();}};c.fn.cycle.transitions={fade:function(t,u,s){u.not(":eq("+s.currSlide+")").css("opacity",0);s.before.push(function(x,v,w){c.fn.cycle.commonReset(x,v,w);
